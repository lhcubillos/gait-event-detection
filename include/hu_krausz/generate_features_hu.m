function [X, t_window] = generate_features_hu(t, imu_accel,imu_gyro, sample_rate, wl, stp)
...
% TODO: check if can use same values
filt_gyro = filter(b,a, imu_gyro);

% Get orientation angle
FUSE = imufilter('SampleRate',sample_rate);
[orientation, ang_vel] = FUSE(imu_accel*9.81, imu_gyro);
euler = quat2eul(orientation);
% TODO: check if this is the angle we want
or_angle = euler(:,3);
